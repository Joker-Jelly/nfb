...
function downloadFile(){
  ...
  if(f && RoxyFilemanConf.DOWNLOAD){
    ...
    window.frames['frmUpladFile'].location.href = url;
  }
...
function tooltipContent(){
  ...
  if($('#hdViewType').val() == 'thumb' && f.IsImage()){
    ...
    if(RoxyFilemanConf.GENERATETHUMB){
      ...
      imgUrl = RoxyUtils.AddParam(imgUrl, 'width', '350');
      imgUrl = RoxyUtils.AddParam(imgUrl, 'height', '260');
    }
    else
      imgUrl = f.fullPath;
  ...
  if(t == 'thumb'){
    ...
    $('#pnlFileList li').each(function(){
      ...
      if(RoxyFilemanConf.GENERATETHUMB && RoxyUtils.IsImage($(this).attr('data-path'))){
        ...
        imgUrl = RoxyUtils.AddParam(imgUrl, 'width', $(this).children('.icon').css('width'));
        imgUrl = RoxyUtils.AddParam(imgUrl, 'height', $(this).children('.icon').css('height'));
      }
